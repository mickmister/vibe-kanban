...
COPY crates/tauri-app/ crates/tauri-app/
...
ARG VK_CARGO_FEATURES=""

RUN --mount=type=cache,id=cargo-registry,target=/usr/local/cargo/registry \
    --mount=type=cache,id=cargo-git,target=/usr/local/cargo/git \
    --mount=type=cache,id=workspace-target,target=/app/target \
    if [ -n "${VK_CARGO_FEATURES}" ]; then \
      VK_BUILD_COMMIT_HASH="${VK_BUILD_COMMIT_HASH}" cargo build --locked --release --bin server --features "${VK_CARGO_FEATURES}"; \
    else \

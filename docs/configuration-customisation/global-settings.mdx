...
Configure automatic cleanup for old forked provider sessions.

When cleanup is enabled, Vibe Kanban keeps the newest provider session IDs for each Vibe Kanban session and coding agent, then cleans up older forked sessions in the background. This reduces local Codex, Claude Code, and OpenCode session storage while preserving a recent undo cushion.

### Cleanup controls

- **Enable automatic session cleanup**: Turn off this setting if you prefer to keep all provider session history.
- **Sessions to keep**: Set how many newest unique provider session IDs Vibe Kanban keeps per session and agent before old forked sessions become cleanup candidates.

<Warning>
Session cleanup deletes provider session state. Increase the retention count or disable cleanup if you often resume older forked sessions.
</Warning>
